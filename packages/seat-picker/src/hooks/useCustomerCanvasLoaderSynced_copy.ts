...
export const useCustomerCanvasLoaderSynced = ({
    ...
    const selectSeat = (target: any, canvas: fabric.Canvas) => {
        ...
        // Check status: if blocked/sold/held, do NOT apply visuals (border/icon)
        // But we still mark it as selected (above) so it gets added to the list.
            ...
            () => {
                ...
                if (true) { // Always enforce strict customer view (only selection, no moving)
                    ...
                    canvas.getObjects().forEach((obj: any) => {
                        ...
                        // MODIFIED LOGIC: Allow ALL seats to be evented regardless of availability

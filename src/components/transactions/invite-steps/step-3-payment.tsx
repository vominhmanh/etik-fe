        ...
        <Grid xs={12} lg={7}>
          <Stack spacing={3}>
            ...
            <Card>
                ...
                />
                <CardContent sx={{ pb: invitationSettings.applyVoucherAsDefault ? 0 : 2 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                    <input 
                      type="checkbox" 
                      checked={invitationSettings.applyVoucherAsDefault}
                      onChange={(e) => {
                        setInvitationSettings({...invitationSettings, applyVoucherAsDefault: e.target.checked});
                        if (!e.target.checked && appliedVoucher) {
                          onRemoveAppliedVoucher();
                        }
                      }}
                    />
                    <Typography variant="body2" sx={{ fontWeight: 500 }}>
                      Áp dụng sẵn mã khuyến mãi cho khách hàng (Mã đang áp dụng: {appliedVoucher?.code || 'Chưa áp dụng'})
                    </Typography>
                  </label>
                ...
                {invitationSettings.applyVoucherAsDefault && (
                  <>
                    {!appliedVoucher && (
                      <Box sx={{ px: 3, pb: 3, pt: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'stretch', gap: 1.5, maxWidth: 360 }}>
                      <OutlinedInput
                        name="discountCode"
                        ...
                        startAdornment={
                          <InputAdornment position="start">
                            <TicketIcon size={18} weight="duotone" style={{ opacity: 0.6 }} />
                          </InputAdornment>
                        }
                      />
                      <Button
                ...
                {(appliedVoucher || availableVouchers.length > 0) && (
                  <>
                    <Divider />
                    <CardContent sx={{ maxHeight: '300px', overflowY: 'auto', py: 2, px: 3 }}>
                      {appliedVoucher ? (
                        <Box
                          sx={{
                            display: 'flex',
                            ...
                            borderLeftColor: voucherValidation.valid ? 'success.main' : 'error.main',
                            borderRadius: '6px',
                        ...
                        >
                          <Box sx={{ flex: 1 }}>
                            <Stack spacing={0.25}>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <Typography variant="subtitle2" sx={{ fontWeight: 600, color: voucherValidation.valid ? 'success.main' : 'error.main' }}>
                              ...
                              {!voucherValidation.valid && (
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                                  <Typography variant="caption" color="error.main" sx={{ fontWeight: 500 }}>
                                    {tt('Mã không hợp lệ', 'Invalid code')}
                ...
                <CardHeader
                  ...
                  subheader={
                    invitationSettings.letCustomerSelect 
                      ? tt("Khách sẽ thanh toán khi chọn vé", "Customer will pay when selecting tickets")
                      : tt("Đơn hàng 0đ, không cần thanh toán", "0đ order, no payment required")
                  }

...
import {
  Box, Button, Card, CardHeader, Checkbox, Divider, FormControlLabel,
  Grid, List, ListItem, MenuItem, Select, Stack, TextField, Typography
} from "@mui/material";
import { AxiosResponse } from "axios";
import React, { useContext, useEffect, useMemo, useState } from "react";

/** ---------------- Types ---------------- */
interface SelectedComponent { key: string; label: string; }
...
interface LabelSize { value: string; label: string; width: number; height: number; }
...
export default function Page({ params }: { params: { event_id: number } }): React.JSX.Element {
  useEffect(() => { document.title = "Thiết kế tem nhãn | ETIK - Vé điện tử & Quản lý sự kiện"; }, []);
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        setIsLoading(true);
        const response: AxiosResponse<TicketTagSettings> =
          await baseHttpServiceInstance.get(`/event-studio/events/${event_id}/ticket-tag-settings`);
        if (response.status === 200) {
          const { size, selectedComponents, componentSettings } = response.data;
          setSelectedSize(size || "50x30mm");
          setSelectedComponents(
            selectedComponents.reduce((acc, c) => { acc[c.key] = c; return acc; }, {} as Record<string, SelectedComponent>)
          );
  ...
  const handleSaveTemplateSettings = async () => {
    try {
      ...
      const response: AxiosResponse =
        await baseHttpServiceInstance.post(`/event-studio/events/${event_id}/ticket-tag-settings`, payload);
    ...
    setSelectedComponents(prev => {
      ...
      if (next[key]) delete next[key];
        ...
        if (!componentSettings[key]) {
          setComponentSettings(ps => ({ ...ps, [key]: { width: 20, height: 10, top: 10, left: 10, fontSize: 10, color: "000000" } }));
        }
      }
      return next;
    });
  ...
  const currentSize = useMemo(
    () => labelSizes.find(s => s.value === selectedSize) || labelSizes[0],
    [selectedSize]
  );
  ...
  const contentHeightMm = useMemo(() => {
    const maxBottom = Object.values(selectedComponents).reduce((mx, { key }) => {
      const s = componentSettings[key]; if (!s) return mx;
  ...
  const previewScale = Math.min(
    PREVIEW_MAX_WIDTH_PX / (contentWidthMm * PX_PER_MM),
    1
  );
    ...
    <>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} md={3}>
          <Box>
            <Typography variant="body2" sx={{ mb: 1 }}>Chọn kích thước tem nhãn:</Typography>
            <Select
              value={selectedSize}
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
              <Button variant="outlined" size="small" onClick={applyDefaultTemplate} sx={{ fontSize: "0.75rem" }} fullWidth>
              ...
              <div style={{ position: "absolute", inset: 0 }}>
                {Object.values(selectedComponents).map(({ key, label }) => {
                  ...
                  if (key === "eCodeQr" && isPrintMode && previewData[key]) {
                    return (
                      <img key={key} src={previewData[key]} alt="QR Code"
                        style={{ ...styleBase, objectFit: "contain" }} className="print-component" />
                    );
                  }
          ...
          <Stack spacing={3}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
              {Object.keys(previewData).length > 0 && (
                <Button variant="outlined" color="secondary" onClick={() => setPreviewData({})} disabled={isLoading} size="small">
                  Quay lại thiết kế
                </Button>
              )}

...
const EditableGrid: FC<EditableGridProps> = ({ shows = [] }) => {
  ...
  const handleDeleteSheet = async () => {
    const confirmText =
      'CẢNH BÁO: Hành động này sẽ xóa toàn bộ danh sách của game này và HỦY toàn bộ vé đã sinh ra.\nKhách hàng đã nhận email chứa vé sẽ không thể dùng vé cũ để check-in nữa.\nBạn có chắc chắn muốn XÓA KHÔNG?';
    if (!window.confirm(confirmText)) return;
    ...
    try {
      await baseHttpServiceInstance.delete(
        `/special_events/tft-2025/sheets/${sheet.showId}`
      );
      notificationCtx.success('Đã xóa bảng đấu và hủy vé thành công');

      // update local state: mark saved: false, empty userList
      setSheets(prev => {
        const copy = [...prev];
        copy[current] = {
          ...copy[current],
          saved: false,
          userList: makeEmpty(copy[current].headers.length)
        };
        return copy;
      });
    ...
    <Box sx={{ width: '100%', p: 2 }}>
      ...
      {!active.saved ? (
        ...
        <Box sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end' }}>
          <Button variant="outlined" color="error" onClick={handleDeleteSheet}>
            Xóa bảng đấu & Hủy vé
          </Button>
        </Box>
      )}
